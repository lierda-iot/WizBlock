...
> 本索引由 `examples.yml` 的 S0 审计结果生成。`publish`/`publish-with-limitations` 表示可进入本地公开候选，不代表主许可证或正式发布门禁已经完成。

## 可进入公开候选

| 编号 | 目录 | 公开状态 | 当前验证状态 | 主要限制 |
| --- | --- | --- | --- | --- |
...
通用构建方法见 [CODE/README.md](../README.md)。本阶段不修改或统一本地构建脚本。
