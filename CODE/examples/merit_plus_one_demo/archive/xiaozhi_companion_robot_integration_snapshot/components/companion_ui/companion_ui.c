                           ...
                           const companion_ui_state_t *second)
{
    return first->product_state == second->product_state &&
           ...
           first->merit_bubble_active == second->merit_bubble_active &&
           first->merit_bubble_epoch == second->merit_bubble_epoch &&
           first->merit_bubble_start_ms == second->merit_bubble_start_ms &&
           first->merit_bubble_repeat_count ==
               second->merit_bubble_repeat_count;
}
...
static void ui_task(void *arg)
{
    ...
    companion_expression_signals_t expression_signals = {
        ...
        .touch_active = false,
        .merit_bubble = {
            .active = current_state.merit_bubble_active,
            .epoch = current_state.merit_bubble_epoch,
            .start_ms = current_state.merit_bubble_start_ms,
        },
    };
    ...
    while (s_available) {
        ...
        if (NULL != s_state_lock &&
            ...
            if (!ui_state_equal(&current_state, &s_pending_state)) {
                ...
                expression_signals.merit_bubble = (companion_merit_bubble_signal_t){
                    .active = current_state.merit_bubble_active,
                    .epoch = current_state.merit_bubble_epoch,
                    .start_ms = current_state.merit_bubble_start_ms,
                };
...
touch_poll_complete:
        {
        if (expression_signals.merit_bubble.active && now_ms >=
            expression_signals.merit_bubble.start_ms + 800U) {
            expression_signals.merit_bubble.active = false;
            render_requested = true;
        }
        const uint32_t animation_interval_ms =
            expression_signals.merit_bubble.active ? 10U :
                                                       COMPANION_UI_ANIMATION_MS;
        if (render_requested ||
            (now_ms - last_animation_ms) >= animation_interval_ms) {
