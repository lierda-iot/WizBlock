...
#define CONTROLLER_AGENT_AUDIO_LOCK_TIMEOUT_MS 100U

typedef enum {
    CONTROLLER_EVENT_STARTUP_COMPLETE = 0,
    CONTROLLER_EVENT_AUDIO_WAKE,
    CONTROLLER_EVENT_VAD_START,
    CONTROLLER_EVENT_SPEECH_CONFIRMED,
...
static network_manager_snapshot_t s_network_snapshot;
static network_manager_snapshot_t s_network_latest_snapshot;
...
static companion_controller_tts_activity_fact_t s_tts_activity_latest;
static uint32_t s_tts_activity_applied_revision;
...
static SemaphoreHandle_t s_agent_audio_lock;
...
typedef struct {
    controller_agent_binding_t binding;
    const char *reason;
} controller_agent_stop_context_t;

typedef struct {
    companion_product_state_t before;
    bool requires_localization;
    esp_err_t wake_result;
} controller_wake_effect_context_t;
...
static esp_err_t model_finish_agent(uint32_t generation,
                                    uint32_t wake_seq,
                                    uint64_t timestamp_ms)
{
    const companion_controller_input_t input = {
...
static esp_err_t model_on_vad_end(uint32_t generation, uint32_t wake_seq,
                                  bool agent_listening, bool *notify_agent)
{
...
static esp_err_t model_on_speech_confirmed(
    uint32_t generation, uint32_t wake_seq, uint64_t timestamp_ms)
{
    const companion_controller_input_t input = {
...
static esp_err_t model_on_vad_start(uint32_t generation, uint32_t wake_seq,
                                    bool agent_listening, bool *notify_agent)
{
    if (NULL == notify_agent) {
        return ESP_ERR_INVALID_ARG;
...
static bool model_token_is_current(uint32_t generation, uint32_t wake_seq,
                                   companion_controller_snapshot_t *snapshot)
{
    ...
    companion_controller_snapshot_t current = {0};
    companion_controller_model_snapshot(&s_model, &current);
    xSemaphoreGive(s_model_lock);
    if (NULL != snapshot) {
        *snapshot = current;
    }
    return current.generation == generation && current.wake_seq == wake_seq;
}
...
static const char *deadline_reason_name(
    companion_controller_deadline_reason_t reason)
{
    switch (reason) {
    case COMPANION_CONTROLLER_DEADLINE_STATE: return "state";
    case COMPANION_CONTROLLER_DEADLINE_MOTION: return "motion";
    case COMPANION_CONTROLLER_DEADLINE_SPEAKING_INACTIVITY:
        return "speaking_inactivity";
    case COMPANION_CONTROLLER_DEADLINE_SPEAKING_ABSOLUTE:
        return "speaking_absolute";
    default: return "unknown";
    }
}
...
static void sync_ui(void)
{
    ...
    portEXIT_CRITICAL(&s_fact_lock);
    if (!ui_available || NULL == s_model_lock) {
        return;
    }
    ...
    state.look_direction =
        (COMPANION_CONTROLLER_MOTION_TURNING == snapshot.motion_state) ?
...
static esp_err_t retire_agent_binding_for_stop(void *user_ctx)
{
    const controller_agent_stop_context_t *context = user_ctx;
    if (NULL == context) {
        return ESP_ERR_INVALID_ARG;
    }
    return companion_agent_adapter_retire_binding(
        context->binding.generation, context->binding.wake_seq,
        context->binding.request_id);
}

static esp_err_t retire_controller_binding_for_stop(void *user_ctx)
{
    const controller_agent_stop_context_t *context = user_ctx;
    if (NULL == context) {
        return ESP_ERR_INVALID_ARG;
    ...
    clear_agent_binding();
    ESP_LOGI(TAG,
             "[DEBUG-AI-P0] phase=controller_binding_retired generation=%lu wake_seq=%lu epoch=%lu request=%lu reason=%s",
             (unsigned long)context->binding.generation,
             (unsigned long)context->binding.wake_seq,
             (unsigned long)context->binding.session_epoch,
             (unsigned long)context->binding.request_id,
             (NULL != context->reason) ? context->reason : "?");
    return ESP_OK;
}

static esp_err_t cancel_agent_request_for_stop(void *user_ctx)
{
    const controller_agent_stop_context_t *context = user_ctx;
    if (NULL == context) {
        return ESP_ERR_INVALID_ARG;
    }
    return companion_agent_adapter_cancel(
        context->binding.generation, context->binding.wake_seq,
        context->binding.request_id);
}

static esp_err_t stop_audio_for_stop(void *user_ctx)
{
    const controller_agent_stop_context_t *context = user_ctx;
    if (NULL == context) {
        return ESP_ERR_INVALID_ARG;
    }
    const esp_err_t result = stop_audio_output_for_binding(&context->binding);
    ESP_LOGI(TAG,
             "[DEBUG-AI-P0] phase=audio_stop generation=%lu wake_seq=%lu request=%lu result=%s",
             (unsigned long)context->binding.generation,
             (unsigned long)context->binding.wake_seq,
             (unsigned long)context->binding.request_id,
             esp_err_to_name(result));
...
static void stop_runtime_effects_with_plan(
    const char *reason, companion_controller_stop_reason_t stop_reason)
{
    companion_controller_stop_plan_t stop_plan = {0};
    if (ESP_OK != companion_controller_stop_plan_build(stop_reason,
                                                       &stop_plan)) {
        stop_reason = COMPANION_CONTROLLER_STOP_SESSION;
        (void)companion_controller_stop_plan_build(stop_reason, &stop_plan);
    }
    bool audio_locked = false;
    if (NULL != s_agent_audio_lock) {
        audio_locked = pdTRUE == xSemaphoreTake(s_agent_audio_lock,
                                                 portMAX_DELAY);
    }
    const controller_agent_binding_t binding = agent_binding_snapshot();
    close_upload_gate();
    if (0U != binding.request_id) {
        const controller_agent_stop_context_t context = {
            .binding = binding,
            .reason = reason,
        };
        const companion_controller_agent_stop_ops_t ops = {
            .retire_controller_binding = retire_controller_binding_for_stop,
            .retire_binding = retire_agent_binding_for_stop,
            .cancel_request = cancel_agent_request_for_stop,
            .stop_audio = stop_audio_for_stop,
        };
        const esp_err_t agent_stop_result =
            companion_controller_agent_stop_execute(
                stop_reason, &ops, (void *)&context);
        ESP_LOGI(TAG,
                 "[DEBUG-AI-P0] phase=agent_stop_plan generation=%lu wake_seq=%lu request=%lu transport=%s result=%s",
                 (unsigned long)binding.generation,
                 (unsigned long)binding.wake_seq,
                 (unsigned long)binding.request_id,
                 (COMPANION_CONTROLLER_AGENT_STOP_RETIRE_BINDING ==
                  stop_plan.agent_action) ? "preserved" : "closed",
                 esp_err_to_name(agent_stop_result));
    } else {
        clear_agent_binding();
        ESP_LOGI(TAG,
                 "[DEBUG-AI-P0] phase=controller_binding_retired generation=0 wake_seq=0 epoch=0 request=0 reason=%s",
                 (NULL != reason) ? reason : "?");
        const esp_err_t audio_stop_result =
            stop_audio_output_for_binding(&binding);
        ESP_LOGI(TAG,
                 "[DEBUG-AI-P0] phase=audio_stop generation=0 wake_seq=0 request=0 result=%s",
                 esp_err_to_name(audio_stop_result));
    }
    if (audio_locked) {
        xSemaphoreGive(s_agent_audio_lock);
    }
    if (stop_plan.cancel_doa && 0U != s_doa_request_id) {
        (void)cancel_doa_safely(s_doa_request_id, reason);
        s_doa_request_id = 0U;
    }
    if (stop_plan.stop_motion) {
...
{
    stop_runtime_effects_with_plan(reason,
                                   COMPANION_CONTROLLER_STOP_SESSION);
}

static void stop_agent_plane_effects(const char *reason)
{
    stop_runtime_effects_with_plan(
        reason, COMPANION_CONTROLLER_STOP_AGENT_PLANE);
}
...
static bool cancel_model_session_for_token(uint32_t generation,
                                           uint32_t wake_seq,
                                           const char *reason)
{
    ...
    if (!cancelled) {
        ESP_LOGI(TAG,
                 "[DEBUG-AI-P0] phase=cancel_skip_superseded generation=%lu/%lu wake_seq=%lu/%lu reason=%s",
                 (unsigned long)generation,
                 (unsigned long)current.generation,
                 (unsigned long)wake_seq,
                 (unsigned long)current.wake_seq,
                 (NULL != reason) ? reason : "?");
        return false;
    }
    s_next_roam_ms = now_ms() + s_config.roam_config.initial_delay_ms;
...
static bool cancel_session_for_token(uint32_t generation, uint32_t wake_seq,
                                     const char *reason)
{
    companion_controller_snapshot_t current = {0};
    if (!model_token_is_current(generation, wake_seq, &current)) {
        ESP_LOGI(TAG,
                 "[DEBUG-AI-P0] phase=cancel_skip_superseded generation=%lu/%lu wake_seq=%lu/%lu state=%s reason=%s",
                 (unsigned long)generation,
                 (unsigned long)current.generation,
                 (unsigned long)wake_seq,
                 (unsigned long)current.wake_seq,
                 state_name(current.product_state),
                 (NULL != reason) ? reason : "?");
        return false;
    }
    stop_runtime_effects(reason);
    return cancel_model_session_for_token(generation, wake_seq, reason);
}

static bool finish_agent_for_token(uint32_t generation, uint32_t wake_seq,
                                   const char *reason)
{
    companion_controller_snapshot_t current = {0};
    if (!model_token_is_current(generation, wake_seq, &current)) {
        ESP_LOGI(TAG,
                 "[DEBUG-AI-P0] phase=agent_cleanup_skip_superseded generation=%lu/%lu wake_seq=%lu/%lu reason=%s",
                 (unsigned long)generation,
                 (unsigned long)current.generation,
                 (unsigned long)wake_seq,
                 (unsigned long)current.wake_seq,
    ...
    const esp_err_t result = model_finish_agent(generation, wake_seq,
                                                now_ms());
    xSemaphoreGive(s_model_lock);
    if (ESP_OK != result) {
...
static bool validate_runtime_invariants(void)
{
    ...
    network_ready = s_network_ready;
    applied_network_ready = s_network_snapshot.stable_ready;
    ...
    } else if (COMPANION_CONTROLLER_MOTION_LOCATING ==
                   model_snapshot.motion_state &&
               0U == s_doa_request_id) {
        violation = "LOCATING without DOA request";
    } else if (0U != s_doa_request_id &&
               COMPANION_CONTROLLER_MOTION_LOCATING !=
                   model_snapshot.motion_state) {
        violation = "DOA request outside owner state";
    } else if (COMPANION_CONTROLLER_MOTION_TURNING ==
                   model_snapshot.motion_state &&
               (0U == s_wake_motion_request_id ||
                COMPANION_TURN_NONE == s_look_direction)) {
        violation = "TURNING without WAKE_TURN";
    } else if (0U != s_wake_motion_request_id &&
               COMPANION_CONTROLLER_MOTION_TURNING !=
                   model_snapshot.motion_state) {
        violation = "WAKE_TURN outside owner state";
    } else if (!companion_controller_roam_runtime_is_valid(
                   state, roam_requested, wake_reserved,
                   CONTROLLER_ROAM_RUNNING == s_roam_state)) {
        violation = (COMPANION_PRODUCT_IDLE == state && roam_requested &&
                     wake_reserved) ?
                    "Roam running while wake reservation pending" :
                    "Roam running while gate closed";
...
static void handle_emergency_exit(void)
{
    ...
    if (!network_ready &&
        (wake_reserved ||
         companion_controller_motion_state_is_active(motion_state) ||
         COMPANION_PRODUCT_CONNECTING == state ||
         COMPANION_PRODUCT_LISTENING == state ||
         COMPANION_PRODUCT_PROCESSING == state ||
         COMPANION_PRODUCT_SPEAKING == state)) {
    ...
    const bool session_active = wake_reserved ||
        COMPANION_PRODUCT_CONNECTING == state ||
        COMPANION_PRODUCT_LISTENING == state ||
        COMPANION_PRODUCT_PROCESSING == state ||
        COMPANION_PRODUCT_SPEAKING == state;
    if (!session_active &&
        COMPANION_CONTROLLER_MOTION_LOCATING == motion_state) {
        ...
        if (ESP_OK == result) {
            ...
            (void)cancel_session_for_token(
                generation, wake_seq, "LOCATING emergency fallback failed");
        }
        return;
    }
    if (!session_active &&
        COMPANION_CONTROLLER_MOTION_TURNING == motion_state) {
        ...
        if (ESP_OK == result) {
            ...
            (void)cancel_session_for_token(
                generation, wake_seq, "TURNING emergency fallback failed");
        }
        return;
    }
    if (session_active ||
        companion_controller_motion_state_is_active(motion_state)) {
        stop_runtime_effects("controller critical event delivery failed");
        companion_controller_snapshot_t current = {0};
        ...
        if (!cancel_current) {
            ESP_LOGI(TAG,
                     "[DEBUG-AI-P0] phase=emergency_skip_superseded expired=%lu/%lu current=%lu/%lu reserved=%u",
                     (unsigned long)generation, (unsigned long)wake_seq,
                     (unsigned long)current.generation,
                     (unsigned long)current.wake_seq,
                     current.wake_reserved ? 1U : 0U);
            return;
        }
        s_next_roam_ms = now_ms() + s_config.roam_config.initial_delay_ms;
        return;
...
static esp_err_t notify_agent(uint32_t generation, uint32_t wake_seq)
{
    ...
    if (ESP_OK != result) {
        s_stats.module_errors++;
        stop_agent_plane_effects("agent wake notify failed");
        (void)finish_agent_for_token(generation, wake_seq,
                                     "agent wake notify failed");
        return result;
    }
    if (pdTRUE == xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
    ...
    if (ESP_OK != result) {
        ...
        s_stats.stale_events++;
        stop_agent_plane_effects("agent accept model mismatch");
        (void)finish_agent_for_token(generation, wake_seq,
                                     "agent accept model mismatch");
        return result;
    }
...
static esp_err_t submit_wake_turn(
    const companion_controller_decision_t *decision,
    uint32_t generation, uint32_t wake_seq)
{
    if (NULL == decision ||
    ...
    ESP_LOGW(TAG,
             "wake turn submit skipped generation=%lu wake_seq=%lu request=%lu direction=%d target=%.1f result=%s capability=%u runtime=%u agent_independent=1",
             (unsigned long)generation, (unsigned long)wake_seq,
             (unsigned long)command.request_id,
    ...
    s_look_direction = COMPANION_TURN_NONE;
    companion_controller_decision_t completion = {0};
    esp_err_t completion_result = ESP_ERR_INVALID_STATE;
    if (pdTRUE == xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
        completion_result = model_on_doa(generation, wake_seq, false,
                                         0.0f, &completion);
        xSemaphoreGive(s_model_lock);
    }
    if (ESP_OK == completion_result) {
        apply_decision(&completion, generation, wake_seq);
    } else {
        s_stats.stale_events++;
    }
    return result;
                           ...
                           uint32_t generation, uint32_t wake_seq)
{
    ...
    /* Compatibility-only branch; current model completion paths never emit it. */
    if (COMPANION_CONTROLLER_DECISION_NOTIFY_AGENT == decision->type) {
        (void)notify_agent(generation, wake_seq);
        return;
    }
    if (COMPANION_CONTROLLER_DECISION_START_TURN == decision->type) {
        (void)submit_wake_turn(decision, generation, wake_seq);
    }
}

static esp_err_t start_wake_agent_effect(
    uint32_t generation, uint32_t wake_seq, void *user_ctx)
{
    (void)user_ctx;
    return notify_agent(generation, wake_seq);
}

static esp_err_t complete_wake_motion_without_turn(
    uint32_t generation, uint32_t wake_seq)
{
    companion_controller_decision_t decision = {0};
    esp_err_t result = ESP_ERR_INVALID_STATE;
    if (pdTRUE == xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
        result = model_on_doa(generation, wake_seq, false, 0.0f, &decision);
        xSemaphoreGive(s_model_lock);
    }
    if (ESP_OK == result) {
        apply_decision(&decision, generation, wake_seq);
...
static esp_err_t start_wake_motion_effect(
    uint32_t generation, uint32_t wake_seq, void *user_ctx)
{
    const controller_wake_effect_context_t *context =
        (const controller_wake_effect_context_t *)user_ctx;
    if (NULL == context) {
        return ESP_ERR_INVALID_ARG;
    }
    if (!context->requires_localization) {
        return ESP_OK;
    }
    ...
    esp_err_t effect_result = context->wake_result;
    if (ESP_OK == effect_result && doa_available) {
        uint32_t request_id = 0U;
        effect_result = companion_doa_request_ex(generation, wake_seq,
                                                 &request_id);
        if (ESP_OK == effect_result) {
            if (pdTRUE == xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
                effect_result = model_mark_locating_started(
                    generation, wake_seq, now_ms());
                xSemaphoreGive(s_model_lock);
            } else {
                effect_result = ESP_ERR_TIMEOUT;
            }
        }
        if (ESP_OK == effect_result) {
            s_doa_request_id = request_id;
            ESP_LOGI(TAG,
                     "[DEBUG-AI-P0] phase=doa_submit generation=%lu wake_seq=%lu request=%lu parallel=1 result=%s timestamp_ms=%llu",
                     (unsigned long)generation,
                     (unsigned long)wake_seq,
                     (unsigned long)request_id,
                     esp_err_to_name(effect_result),
                     (unsigned long long)now_ms());
            log_state_transition(context->before, "wake_request_doa",
                                 effect_result);
            return ESP_OK;
        }
        if (0U != request_id) {
    ...
    const esp_err_t cleanup_result =
        complete_wake_motion_without_turn(generation, wake_seq);
    log_state_transition(context->before, "wake_skip_doa", cleanup_result);
    return (ESP_OK != effect_result) ? effect_result : cleanup_result;
}

static void handle_audio_wake(const controller_event_t *event)
{
    const uint64_t wake_detect_ms = now_ms();
    ...
    ESP_LOGI(TAG,
             "[DEBUG-AI-P0] phase=wake_detect generation=%lu wake_seq=%lu source=%s timestamp_ms=%llu",
             (unsigned long)event->generation,
             (unsigned long)event->wake_seq, state_name(before),
             (unsigned long long)wake_detect_ms);
    companion_controller_stop_reason_t stop_reason =
        COMPANION_CONTROLLER_STOP_SESSION;
    if (ESP_OK != companion_controller_wake_stop_reason(before,
                                                        &stop_reason)) {
        ESP_LOGW(TAG,
                 "[DEBUG-AI-P0] phase=wake_stop_plan_fallback source=%s action=cancel",
                 state_name(before));
    }
    stop_runtime_effects_with_plan("wake accepted", stop_reason);
    ESP_LOGI(TAG,
             "[DEBUG-AI-P0] phase=local_stop generation=%lu wake_seq=%lu source=%s elapsed_ms=%llu timestamp_ms=%llu",
             (unsigned long)event->generation,
             (unsigned long)event->wake_seq, state_name(before),
             (unsigned long long)(now_ms() - wake_detect_ms),
             (unsigned long long)now_ms());

    const controller_wake_effect_context_t context = {
        .before = before,
        .requires_localization = requires_localization,
        .wake_result = event->data.wake.result,
    };
    const companion_controller_wake_effect_ops_t ops = {
        .start_agent = start_wake_agent_effect,
        .start_motion = start_wake_motion_effect,
    };
    companion_controller_wake_effect_result_t effect_result = {0};
    const esp_err_t dispatch_result =
        companion_controller_wake_effects_execute(
            &ops, event->generation, event->wake_seq, (void *)&context,
            &effect_result);
    ESP_LOGI(TAG,
             "[DEBUG-AI-P0] phase=wake_effects generation=%lu wake_seq=%lu agent=%s motion=%s dispatch=%s timestamp_ms=%llu",
             (unsigned long)event->generation,
             (unsigned long)event->wake_seq,
             esp_err_to_name(effect_result.agent_result),
             esp_err_to_name(effect_result.motion_result),
             esp_err_to_name(dispatch_result),
             (unsigned long long)now_ms());
    if (ESP_OK != dispatch_result) {
        s_stats.module_errors++;
        request_emergency_exit();
    }
}
...
static void handle_doa(const controller_event_t *event)
{
    companion_controller_decision_t decision = {0};
    companion_product_state_t before = COMPANION_PRODUCT_ERROR;
    companion_controller_snapshot_t current_snapshot = {0};
    esp_err_t result = ESP_ERR_INVALID_STATE;
    if (0U == s_doa_request_id ||
    ...
    companion_controller_model_snapshot(&s_model, &current_snapshot);
    xSemaphoreGive(s_model_lock);
    if (current_snapshot.generation != event->generation ||
        current_snapshot.wake_seq != event->wake_seq ||
        COMPANION_CONTROLLER_MOTION_LOCATING !=
            current_snapshot.motion_state) {
        /* Retire only the matching stale request; never consume a newer
         * transaction's request slot. */
        s_doa_request_id = 0U;
        s_stats.stale_events++;
        ESP_LOGW(TAG,
                 "[DEBUG-AI-P0] phase=doa_drop_stale generation=%lu/%lu wake_seq=%lu/%lu state=%s request=%lu",
                 (unsigned long)event->generation,
                 (unsigned long)current_snapshot.generation,
                 (unsigned long)event->wake_seq,
                 (unsigned long)current_snapshot.wake_seq,
                 state_name(current_snapshot.product_state),
                 (unsigned long)event->data.doa.request_id);
        return;
    }
    ...
    if (pdTRUE == xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
        ...
        before = snapshot.product_state;
        result = model_on_doa(
            event->generation, event->wake_seq, doa_valid,
    ...
    s_doa_request_id = 0U;
    ESP_LOGI(TAG,
             "[DEBUG-AI-P0] phase=doa_done generation=%lu wake_seq=%lu parallel=1 valid=%u result=%s turn=%d timestamp_ms=%llu",
             (unsigned long)event->generation,
             (unsigned long)event->wake_seq, doa_valid ? 1U : 0U,
             esp_err_to_name(result), (int)decision.turn.direction,
             (unsigned long long)now_ms());
...
static void handle_motion_progress(const controller_event_t *event)
{
    ...
    companion_controller_snapshot_t snapshot = {0};
    if (pdTRUE != xSemaphoreTake(s_model_lock, pdMS_TO_TICKS(20U))) {
        s_stats.stale_events++;
        return;
    }
    companion_controller_model_snapshot(&s_model, &snapshot);
    xSemaphoreGive(s_model_lock);
    if (COMPANION_CONTROLLER_MOTION_TURNING != snapshot.motion_state ||
...
static void handle_motion_done(const controller_event_t *event)
{
    ...
    if (snapshot.generation != event->generation ||
        snapshot.wake_seq != event->wake_seq) {
        if (COMPANION_MOTION_ROLE_WAKE_TURN ==
            event->data.motion.command.role &&
    ...
    esp_err_t result = ESP_ERR_INVALID_STATE;
    if (pdTRUE == xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
        result = model_on_motion_done(event->generation,
                                      event->wake_seq, &decision);
        xSemaphoreGive(s_model_lock);
    }
    if (ESP_OK != result) {
        s_stats.stale_events++;
    ...
    if (COMPANION_MOTION_RESULT_PERMANENT_OUTPUT_FAILURE ==
        ...
        s_stats.module_errors++;
        ESP_LOGW(TAG,
                 "wake turn recoverable generation=%lu wake_seq=%lu request=%lu stage=%d class=%d error=%s availability=%u agent_independent=1",
                 (unsigned long)event->generation,
                 (unsigned long)event->wake_seq,
                 (unsigned long)event->data.motion.command.request_id,
                 (int)event->data.motion.result.failure_stage,
    ...
    ESP_LOGI(TAG,
              "wake turn completion generation=%lu wake_seq=%lu request=%lu stage=%d class=%d error=%s stop=%s availability=%u agent_independent=1",
             (unsigned long)event->generation,
             (unsigned long)event->wake_seq,
              (unsigned long)event->data.motion.command.request_id,
              (int)event->data.motion.result.failure_stage,
             ...
             event->data.motion.result.available ? 1U : 0U);
    ESP_LOGI(TAG,
             "[DEBUG-AI-P0] phase=turn_done generation=%lu wake_seq=%lu request=%lu parallel=1 result=%s timestamp_ms=%llu",
             (unsigned long)event->generation,
             (unsigned long)event->wake_seq,
             (unsigned long)event->data.motion.command.request_id,
             esp_err_to_name(event->data.motion.result.error),
...
static void handle_agent_semantic(const controller_event_t *event)
{
    ...
    bool refresh_upload_gate = false;
    if (pdTRUE == xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
        companion_controller_snapshot_t snapshot = {0};
        companion_controller_model_snapshot(&s_model, &snapshot);
        before = snapshot.product_state;
    ...
    if (COMPANION_AGENT_EVENT_LISTENING_READY == agent->type) {
        ESP_LOGI(TAG,
                 "[DEBUG-AI-P0] phase=listen_ready generation=%lu wake_seq=%lu request=%lu epoch=%lu timestamp_ms=%llu",
                 (unsigned long)agent->generation,
                 (unsigned long)agent->wake_seq,
                 (unsigned long)agent->request_id,
                 (unsigned long)agent->session_epoch,
                 (unsigned long long)now_ms());
    }
    if (notify_pending_vad) {
        ...
        if (ESP_OK != vad_result) {
            stop_agent_plane_effects("pending VAD notify failed");
            (void)finish_agent_for_token(
                agent->generation, agent->wake_seq,
                "pending VAD notify failed");
        }
    }
    if (COMPANION_AGENT_EVENT_CLOSED == agent->type ||
        COMPANION_AGENT_EVENT_FAILED == agent->type) {
        stop_agent_plane_effects("agent transaction closed");
        (void)finish_agent_for_token(agent->generation,
                                     agent->wake_seq,
                                     "agent transaction closed");
    }
    log_state_transition(before, "agent_semantic", result);
}

static void handle_vad_start(const controller_event_t *event)
{
    bool notify_agent = false;
    bool agent_listening = false;
    companion_controller_snapshot_t snapshot = {0};
    if (pdTRUE != xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
        s_stats.stale_events++;
        return;
    ...
    const controller_agent_binding_t binding = agent_binding_snapshot();
    if (COMPANION_PRODUCT_LISTENING == snapshot.product_state) {
        agent_listening = true;
    } else if (binding.generation == event->generation &&
               binding.wake_seq == event->wake_seq &&
               0U != binding.request_id) {
        agent_listening = companion_agent_adapter_is_listening(
            event->generation, event->wake_seq, binding.request_id);
    }
    esp_err_t result = ESP_ERR_INVALID_STATE;
    if (pdTRUE == xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
        result = model_on_vad_start(event->generation, event->wake_seq,
                                    agent_listening, &notify_agent);
        xSemaphoreGive(s_model_lock);
    }
    if (ESP_OK != result) {
        s_stats.stale_events++;
    ...
    if (!notify_agent) {
        ESP_LOGI(TAG,
                 "[DEBUG-AI-P0] phase=vad_start_local generation=%lu wake_seq=%lu state=%s",
                 (unsigned long)event->generation,
                 (unsigned long)event->wake_seq,
    ...
    result = companion_agent_adapter_notify_vad_start(
        event->generation, event->wake_seq, binding.request_id);
    ESP_LOGI(TAG,
             "VAD start generation=%lu wake_seq=%lu request=%lu result=%s",
             (unsigned long)event->generation,
             (unsigned long)event->wake_seq,
             (unsigned long)binding.request_id, esp_err_to_name(result));
    if (ESP_OK != result) {
        (void)cancel_session_for_token(event->generation, event->wake_seq,
                                       "VAD start notify failed");
    }
}

static void handle_speech_confirmed(const controller_event_t *event)
{
    ...
    if (pdTRUE == xSemaphoreTake(s_model_lock, portMAX_DELAY)) {
        result = model_on_speech_confirmed(
            event->generation, event->wake_seq, timestamp_ms);
        companion_controller_model_snapshot(&s_model, &snapshot);
        xSemaphoreGive(s_model_lock);
    }
    if (ESP_OK != result) {
    ...
    ESP_LOGI(TAG,
             "[DEBUG-AI-P0] phase=speech_confirmed generation=%lu wake_seq=%lu state=%s voice_started_ms=%llu deadline_ms=%llu",
             (unsigned long)event->generation,
             (unsigned long)event->wake_seq,
             state_name(snapshot.product_state),
             (unsigned long long)snapshot.listen_voice_started_ms,
             (unsigned long long)snapshot.state_deadline_ms);
}
...
static void handle_vad_end(const controller_event_t *event)
{
    ...
    if (!notify_agent) {
        ESP_LOGI(TAG,
                 "[DEBUG-AI-P0] phase=vad_end_pending generation=%lu wake_seq=%lu state=%s",
                 (unsigned long)event->generation,
                 (unsigned long)event->wake_seq,
                 state_name(snapshot.product_state));
        return;
...
static void handle_network(const controller_event_t *event)
{
    ...
    const bool network_ready = snapshot->stable_ready &&
    ...
    s_network_snapshot.stable_ready = network_ready;
    ...
    ESP_LOGI(TAG,
             ...
             (int)snapshot->lifecycle, (int)snapshot->cellular_phase,
             snapshot->cellular.stable_link_up ? 1U : 0U,
             snapshot->cellular.stable_ipv4_ready ? 1U : 0U,
             snapshot->internet_reachable ? 1U : 0U,
             network_ready ? 1U : 0U,
             (int)snapshot->stable_active_interface, (unsigned long)snapshot->revision,
             (unsigned long)snapshot->cellular.retry_attempt,
             esp_err_to_name(snapshot->last_fault.source_error), state_name(controller_state));
}
...
static void handle_capability(const controller_event_t *event)
{
    ...
    if (!event->data.capability.available) {
        ...
        const companion_controller_motion_state_t motion_state =
            model_before.motion_state;
        ...
        if (COMPANION_CAPABILITY_DOA == capability &&
            ...
            companion_controller_decision_t decision = {0};
            esp_err_t result = ESP_ERR_INVALID_STATE;
            if (pdTRUE == xSemaphoreTake(s_model_lock,
                                         portMAX_DELAY)) {
                result = model_on_doa(generation, wake_seq, false,
                                      0.0f, &decision);
                xSemaphoreGive(s_model_lock);
            }
            if (ESP_OK == result) {
                ...
                (void)cancel_session_for_token(
                    generation, wake_seq,
                    "DOA capability loss fallback failed");
            }
        }
        if (COMPANION_CAPABILITY_MOTION == capability &&
            ...
            if (ESP_OK == result) {
                ...
                (void)cancel_session_for_token(
                    generation, wake_seq,
                    "Motion capability loss fallback failed");
            }
        }
...
static void reconcile_tts_activity(void)
{
    companion_controller_tts_activity_fact_t activity = {0};
    portENTER_CRITICAL(&s_fact_lock);
    if (s_tts_activity_latest.valid &&
        s_tts_activity_latest.revision !=
        s_tts_activity_applied_revision) {
        activity = s_tts_activity_latest;
        s_tts_activity_applied_revision = activity.revision;
    }
    portEXIT_CRITICAL(&s_fact_lock);
    if (!activity.valid) {
        return;
    ...
    const companion_controller_input_t input = {
        ...
        .wake_seq = activity.wake_seq,
        .now_ms = activity.timestamp_ms,
        .data.tts_activity = {
            .session_epoch = activity.session_epoch,
    ...
    if (pdTRUE == xSemaphoreTake(s_model_lock, pdMS_TO_TICKS(20))) {
        result = companion_controller_model_apply(&s_model, &input, &output);
        xSemaphoreGive(s_model_lock);
    }
    if (ESP_OK == result) {
        ESP_LOGI(TAG,
                 "tts activity generation=%lu wake_seq=%lu epoch=%lu request=%lu revision=%lu timestamp_ms=%llu",
                 (unsigned long)activity.generation,
                 (unsigned long)activity.wake_seq,
                 (unsigned long)activity.session_epoch,
                 (unsigned long)activity.request_id,
                 (unsigned long)activity.revision,
                 (unsigned long long)activity.timestamp_ms);
    } else {
        s_stats.stale_events++;
    }
}
...
static void handle_startup_complete(const controller_event_t *event)
{
    ...
    if (ESP_OK != result) {
        close_upload_gate();
        if (NULL != s_agent_audio_lock &&
            pdTRUE == xSemaphoreTake(s_agent_audio_lock, portMAX_DELAY)) {
...
static void handle_deadline(void)
{
    ...
    ESP_LOGE(TAG,
             "state deadline expired state=%s reason=%s generation=%lu wake_seq=%lu request=%lu",
             state_name(expired.state), deadline_reason_name(expired.reason),
             (unsigned long)expired.generation,
    ...
    if (COMPANION_PRODUCT_LOCATING == expired.state) {
        ...
        companion_controller_snapshot_t current = {0};
        if (!model_token_is_current(expired.generation, expired.wake_seq,
                                    &current)) {
            ESP_LOGI(TAG,
                     "[DEBUG-AI-P0] phase=deadline_skip_superseded state=LOCATING expired=%lu/%lu current=%lu/%lu reserved=%u",
                     (unsigned long)expired.generation,
                     (unsigned long)expired.wake_seq,
                     (unsigned long)current.generation,
                     (unsigned long)current.wake_seq,
        ...
        if (ESP_OK == result) {
            ...
            (void)cancel_session_for_token(
                ...
                "LOCATING deadline fallback failed");
        }
        sync_ui();
        return;
    }
    if (COMPANION_PRODUCT_TURNING == expired.state) {
        ...
        if (!model_token_is_current(expired.generation, expired.wake_seq,
                                    &current)) {
            ESP_LOGI(TAG,
                     "[DEBUG-AI-P0] phase=deadline_skip_superseded state=TURNING expired=%lu/%lu current=%lu/%lu reserved=%u",
                     (unsigned long)expired.generation,
                     (unsigned long)expired.wake_seq,
                     (unsigned long)current.generation,
                     (unsigned long)current.wake_seq,
        ...
        if (ESP_OK == result) {
            ...
            (void)cancel_session_for_token(
                expired.generation, expired.wake_seq,
                "TURNING deadline fallback failed");
        }
        sync_ui();
        return;
    }
    if (COMPANION_PRODUCT_CONNECTING == expired.state ||
        ...
        if (!model_token_is_current(expired.generation, expired.wake_seq,
                                    &current)) {
            ESP_LOGI(TAG,
                     "[DEBUG-AI-P0] phase=deadline_skip_superseded state=%s expired=%lu/%lu current=%lu/%lu reserved=%u",
                     state_name(expired.state),
                     (unsigned long)expired.generation,
                     (unsigned long)expired.wake_seq,
                     (unsigned long)current.generation,
                     (unsigned long)current.wake_seq,
        ...
        stop_agent_plane_effects("Agent state deadline");
        if (finish_agent_for_token(expired.generation,
                                   expired.wake_seq,
                                   "Agent state deadline")) {
            s_roam_state = CONTROLLER_ROAM_DELAYED;
        }
    }
    sync_ui();
...
static void schedule_roam(void)
{
    ...
    if (pdTRUE == xSemaphoreTake(s_model_lock, pdMS_TO_TICKS(10))) {
        ...
        wake_reserved = snapshot.wake_reserved;
        wake_motion_active = companion_controller_motion_state_is_active(
            snapshot.motion_state);
        generation = snapshot.generation;
        wake_seq = snapshot.wake_seq;
        xSemaphoreGive(s_model_lock);
    ...
    portEXIT_CRITICAL(&s_fact_lock);
    const bool gate_open = idle && !wake_reserved && !wake_motion_active &&
                           motion_available && input_available;
    if (!gate_open) {
        if (CONTROLLER_ROAM_RUNNING == s_roam_state) {
            s_roam_motion_request_id = 0U;
            (void)companion_motion_stop_role(COMPANION_MOTION_ROLE_ROAM,
...
static void controller_task(void *arg)
{
    ...
    while (true) {
        ...
        if (pdTRUE == xQueueReceive(s_queue, &event,
            ...
            case CONTROLLER_EVENT_VAD_START: handle_vad_start(&event); break;
            case CONTROLLER_EVENT_SPEECH_CONFIRMED:
                handle_speech_confirmed(&event);
                break;
...
esp_err_t companion_controller_start(const companion_controller_config_t *config)
{
    ...
    s_network_snapshot = (network_manager_snapshot_t){0};
    s_network_latest_snapshot = (network_manager_snapshot_t){0};
    s_tts_activity_latest =
        (companion_controller_tts_activity_fact_t){0};
    s_tts_activity_applied_revision = 0U;
    s_agent_recovery_attempted = false;
    ...
    s_agent_audio_lock = xSemaphoreCreateMutex();
    s_queue = xQueueCreate(CONTROLLER_QUEUE_DEPTH, sizeof(controller_event_t));
    if (NULL == s_model_lock || NULL == s_agent_audio_lock ||
        NULL == s_queue) {
        if (NULL != s_queue) {
        ...
        if (NULL != s_agent_audio_lock) {
            vSemaphoreDelete(s_agent_audio_lock);
            s_agent_audio_lock = NULL;
        }
        portENTER_CRITICAL(&s_fact_lock);
        s_starting = false;
        portEXIT_CRITICAL(&s_fact_lock);
...
void companion_controller_on_network_event(
    const network_manager_event_t *event, void *user_ctx)
{
    (void)user_ctx;
    if (NULL == event || 0U == event->sequence) {
        return;
    }
    const network_manager_snapshot_t *snapshot = &event->snapshot;
    if (0U == snapshot->revision) {
        return;
    }
    bool accepted = false;
    ...
    if (snapshot->revision > s_network_latest_snapshot.revision) {
        ...
        s_network_latest_snapshot.stable_ready =
            snapshot->stable_ready &&
            NETWORK_MANAGER_LIFECYCLE_RUNNING == snapshot->lifecycle &&
            NETWORK_MANAGER_INTERFACE_4G == snapshot->stable_active_interface;
        accepted = true;
    }
    latest_ready = s_network_latest_snapshot.stable_ready;
    ...
    const controller_event_t ctrl_event = {
        .type = CONTROLLER_EVENT_NETWORK,
        .data.network.snapshot = *snapshot,
    };
    (void)post_event(&ctrl_event, active_transaction);
}
                                         ...
                                         void *user_ctx)
{
    ...
    } else if (COMPANION_AUDIO_EVENT_VAD_START == event->type) {
        message.type = CONTROLLER_EVENT_VAD_START;
        message.generation = event->generation;
        message.wake_seq = event->wake_seq;
        (void)post_event(&message, true);
    } else if (COMPANION_AUDIO_EVENT_SPEECH_CONFIRMED == event->type) {
        message.type = CONTROLLER_EVENT_SPEECH_CONFIRMED;
        message.generation = event->generation;
        message.wake_seq = event->wake_seq;
        (void)post_event(&message, true);
...
bool companion_controller_get_upload_token(companion_audio_token_t *token,
                                           void *user_ctx)
{
    (void)user_ctx;
    if (NULL == token) {
    ...
    *token = (companion_audio_token_t){
        .generation = s_upload_generation,
        .wake_seq = s_upload_wake_seq,
        .session_epoch = s_agent_session_epoch,
        .request_id = s_upload_request_id,
    };
    portEXIT_CRITICAL(&s_fact_lock);
    if (!gate_open || !audio_token_is_valid(token) ||
        !companion_agent_adapter_is_listening(
            token->generation, token->wake_seq, token->request_id)) {
                                  ...
                                  void *user_ctx)
{
    ...
    request_id = s_upload_request_id;
    portEXIT_CRITICAL(&s_fact_lock);
    const bool token_matches_gate =
        NULL != token && audio_token_is_valid(token) &&
        0U != token->session_epoch && token->generation == generation &&
        token->wake_seq == wake_seq &&
        token->session_epoch == session_epoch &&
        token->request_id == request_id;
    if (NULL == data || 0 >= length || !gate_open || !token_matches_gate ||
        !companion_agent_adapter_is_listening(
            (NULL != token) ? token->generation : 0U,
            (NULL != token) ? token->wake_seq : 0U,
            (NULL != token) ? token->request_id : 0U)) {
        s_stats.upload_drops++;
        if (1U == s_stats.upload_drops ||
            0U == (s_stats.upload_drops % CONTROLLER_UPLOAD_LOG_INTERVAL)) {
            ESP_LOGW(TAG,
                     "[DEBUG-AI-P0] phase=upload_drop encoded=%lu/%lu/%lu/%lu current=%lu/%lu/%lu/%lu gate=%u count=%lu",
                     (unsigned long)((NULL != token) ? token->generation : 0U),
                     (unsigned long)((NULL != token) ? token->wake_seq : 0U),
                     (unsigned long)((NULL != token) ? token->session_epoch : 0U),
                     (unsigned long)((NULL != token) ? token->request_id : 0U),
                     (unsigned long)generation, (unsigned long)wake_seq,
                     (unsigned long)session_epoch,
                     (unsigned long)request_id, gate_open ? 1U : 0U,
                     (unsigned long)s_stats.upload_drops);
        }
        return;
    }
    const esp_err_t result = companion_agent_adapter_send_audio(
        token->generation, token->wake_seq, token->request_id, data, length);
    if (ESP_OK == result) {
    ...
    const companion_agent_audio_event_t *event, void *user_ctx)
{
    ...
    if (NULL == s_agent_audio_lock ||
        pdTRUE != xSemaphoreTake(
            s_agent_audio_lock,
            pdMS_TO_TICKS(CONTROLLER_AGENT_AUDIO_LOCK_TIMEOUT_MS))) {
        s_stats.stale_events++;
        ESP_LOGW(TAG,
                 "[DEBUG-AI-P0] phase=controller_audio_drop request=%lu reason=lock_timeout",
    ...
    if (event->stop) {
        ...
        ESP_LOGI(TAG,
                 "[DEBUG-AI-TXN] phase=audio_stop epoch=%lu request=%lu result=%s",
    ...
    if (ESP_OK == result) {
        ...
        esp_err_t activity_result = ESP_ERR_INVALID_STATE;
        portENTER_CRITICAL(&s_fact_lock);
        activity_result = companion_controller_tts_activity_record(
            &s_tts_activity_latest, token.generation, token.wake_seq,
            token.session_epoch, token.request_id, activity_ms);
        portEXIT_CRITICAL(&s_fact_lock);
        if (ESP_OK != activity_result) {
            s_stats.stale_events++;
        }
    } else {
        s_stats.module_errors++;
...
void companion_controller_get_stats(companion_controller_stats_t *stats)
{
    ...
    *stats = s_stats;
    ...
    stats->network_link_up = s_network_snapshot.cellular.stable_link_up;
    stats->network_ipv4_ready = s_network_snapshot.cellular.stable_ipv4_ready;
    ...
    stats->network_interface = s_network_snapshot.stable_active_interface;
    stats->network_phase = s_network_snapshot.cellular_phase;
    stats->network_revision = s_network_snapshot.revision;
    stats->network_recovery_attempt =
        s_network_snapshot.cellular.retry_attempt;
    stats->network_error = s_network_snapshot.last_fault.source_error;
    portEXIT_CRITICAL(&s_fact_lock);
}
