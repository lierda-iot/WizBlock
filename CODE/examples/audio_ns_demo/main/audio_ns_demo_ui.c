...
static const char *TAG = "audio_ns_demo_ui";
...
static audio_ns_demo_ui_callbacks_t s_callbacks;
...
static lv_color_t ui_state_color(audio_ns_demo_state_t state)
{
    switch (state) {
    case AUDIO_NS_DEMO_STATE_RECORDING:
        return lv_color_make(246, 102, 94);
    case AUDIO_NS_DEMO_STATE_PLAYING_RAW:
        return lv_color_make(245, 181, 74);
    case AUDIO_NS_DEMO_STATE_PLAYING_DENOISED:
        return lv_color_make(74, 192, 218);
    case AUDIO_NS_DEMO_STATE_COMPLETE:
        return lv_color_make(92, 210, 145);
    case AUDIO_NS_DEMO_STATE_ERROR:
        return lv_color_make(255, 112, 112);
    case AUDIO_NS_DEMO_STATE_PREPARING:
    default:
        return lv_color_make(150, 166, 188);
    }
}

static const char *ui_state_text(audio_ns_demo_state_t state)
{
    switch (state) {
    case AUDIO_NS_DEMO_STATE_RECORDING:
        return "RECORDING";
...
static int ui_active_stage(audio_ns_demo_state_t state)
{
    switch (state) {
    case AUDIO_NS_DEMO_STATE_RECORDING:
        return 0;
...
{
    static const char *stage_text[UI_STAGE_COUNT] = {"REC", "RAW", "NS", "DONE"};
    ...
    lv_label_set_text(title, "NS DENOISE COMPARISON");
    ...
    lv_obj_set_style_text_color(s_state_label, ui_state_color(AUDIO_NS_DEMO_STATE_PREPARING), 0);
    ...
    lv_obj_set_style_bg_color(s_progress_bar, ui_state_color(AUDIO_NS_DEMO_STATE_PREPARING), LV_PART_INDICATOR);
    ...
    lv_label_set_text(s_metrics_label, "RAW RMS: --    NS RMS: --");
    lv_obj_set_width(s_metrics_label, UI_HOR_RES - 24);
    lv_obj_set_pos(s_metrics_label, 12, 169);
...
static void ui_format_time(size_t milliseconds, char *text, size_t text_len)
{
    if (NULL == text || 0U == text_len) {
        return;
    }

    snprintf(text, text_len, "%u.%u",
             (unsigned)(milliseconds / 1000U),
             (unsigned)((milliseconds % 1000U) / 100U));
}

static void ui_format_detail(const audio_ns_demo_status_t *status, char *text, size_t text_len)
{
    char completed_text[12] = {0};
    char total_text[12] = {0};

    if (NULL == status || NULL == text || 0U == text_len) {
        return;
    }

    ui_format_time(status->completed_ms, completed_text, sizeof(completed_text));
    ui_format_time(status->total_ms, total_text, sizeof(total_text));

    switch (status->state) {
    case AUDIO_NS_DEMO_STATE_RECORDING:
        snprintf(text, text_len, "%s / %s s", completed_text, total_text);
        break;
    case AUDIO_NS_DEMO_STATE_PLAYING_RAW:
        snprintf(text, text_len, "Original %s / %s s", completed_text, total_text);
        break;
    case AUDIO_NS_DEMO_STATE_PLAYING_DENOISED:
        snprintf(text, text_len, "Denoised %s / %s s", completed_text, total_text);
        break;
    case AUDIO_NS_DEMO_STATE_COMPLETE:
        snprintf(text, text_len, "Comparison complete");
        break;
    case AUDIO_NS_DEMO_STATE_ERROR:
        snprintf(text, text_len, "%s",
                 ('\0' != status->error_text[0]) ? status->error_text : "Audio cycle failed");
        break;
    case AUDIO_NS_DEMO_STATE_PREPARING:
    default:
...
esp_err_t audio_ns_demo_ui_init(const audio_ns_demo_ui_callbacks_t *callbacks)
{
    ...
    task_ok = xTaskCreatePinnedToCore(lvgl_task, "ns_lvgl", UI_TASK_STACK, NULL,
    ...
    s_ui_ready = true;
    ESP_LOGI(TAG, "NS status UI initialized: %dx%d%s",
...
esp_err_t audio_ns_demo_ui_update(const audio_ns_demo_status_t *status)
{
    ...
    if (status->rms_ready) {
        snprintf(metrics_text, sizeof(metrics_text), "RAW RMS: %lu    NS RMS: %lu",
                 (unsigned long)status->raw_rms,
                 (unsigned long)status->denoised_rms);
    } else {
        snprintf(metrics_text, sizeof(metrics_text), "RAW RMS: --    NS RMS: --");
    }

    restart_allowed = audio_ns_demo_restart_allowed(status->state);
    ...
    for (uint32_t i = 0; i < UI_STAGE_COUNT; i++) {
        ...
        if (AUDIO_NS_DEMO_STATE_COMPLETE == status->state ||
            (0 <= active_stage && (int)i < active_stage)) {
            stage_color = ui_state_color(AUDIO_NS_DEMO_STATE_COMPLETE);
    ...
    if (restart_allowed) {
        ...
        lv_label_set_text(s_restart_button_label,
                          (AUDIO_NS_DEMO_STATE_ERROR == status->state)
                              ? LV_SYMBOL_REFRESH " Retry"
                              : LV_SYMBOL_REFRESH " Run again");
